...
function chat_write($write) {
  ...
  $time = date('H:i:s');
  $js = "cs($offset,$USER_ID,'$time','".$_SESSION['s_personaje_nombre']."','$write','');\n";
  // Go to the top
  fseek($f, 0);
  // Empty the file
  ftruncate($f, 0);
...
function cs(o, i, t, u, m, c) {
  if (m == '<E>') {
    if (u != '') messages[o] = '<span id="C'+i+'">['+t+'] * '+u+' ha entrado a Niburu Online. *</span><br />';
  } else if (m == '<L>') {
    if (u != '') messages[o] = '<span id="C'+i+'">['+t+'] * '+u+' ha salido de Niburu. *</span><br />';
  } else {
    if (u != '') {
      u += ':';
      spaces = 5 - u.length;
